...
const JobsPage = () => {
  ...
  useEffect(() => {
    const applyFilters = () => {
      handleFilters(allJobs, selectedJobFunction, selectedJobType, searchLocation, searchTitle);
    };

    if (allJobs.length > 0) {
      applyFilters();
    }
  }, [allJobs, selectedJobFunction, selectedJobType, searchLocation, searchTitle]);
  ...
  const handleFilters = (
    ...
    if (selectedJobFunctions.length === 0 && selectedJobTypes.length === 0 && !location && !title) {
    ...
    const filtered = jobs.filter((job) => {
      ...
      return matchesJobFunction && matchesJobType && matchesLocation && matchesTitle;
    });
  ...
  }, [selectedJobFunction, selectedJobType, searchLocation, searchTitle]);
